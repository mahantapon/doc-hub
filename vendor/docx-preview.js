                ...
                switch (c.localName) {
                    case "t":
                        result.children.push({
                            type: DomType.Text,
                            text: c.textContent,
                            /* DocHub patch: sequential id for edit-in-preview mapping */
                            __tid: window.__dxTexts ? (window.__dxTexts.push(c.textContent) - 1) : undefined
                        });
                        break;
                    case "delText":
                        result.children.push({
...
`;
        ...
        renderText(elem) {
            /* DocHub patch: wrap tagged text in a span carrying its source id */
            if (elem.__tid !== undefined) {
                const s = this.h({ tagName: "span", className: "dx-t", children: [elem.text] });
                s.__dxTid = elem.__tid;
                return s;
            }
